...
class PDFController extends Controller
{
    ...
    public function reciboPDF(string $id)
    {
        ...
        if (!$tramite) {
            session()->flash('warning', '¡El trámite no existe!');
    ...
    public function reportePDF(string $tipo, Request $request)
    {
        if ($tipo === 'rango' && (!$request->fechaInicio || !$request->fechaFin)) {
            session()->flash('error', '¡Debes seleccionar un rango de fechas!');
            return redirect()->back();
        }
        ...
        if ($reporte['tramites']->isEmpty()) {
            session()->flash('warning', '¡No hay trámites registrados en esta fecha!');
            return redirect()->back();
        }
